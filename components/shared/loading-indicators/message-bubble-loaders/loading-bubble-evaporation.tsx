// loading-bubble-evaporation.tsx
...
const AnimatedLetter = React.memo(
  ({
    ...
    useEffect(() => {
      // Math Magic: Calculates delay from the edges moving inward to the center
      const distanceFromEdge = Math.min(index, totalChars - 1 - index);
      const delay = distanceFromEdge * 35;

      if (isPrev) {
        progress.value = withDelay(delay, withTiming(1, { duration: 300 }));
      } else {
      ...
      if (isPrev) {
        ...
        return {
          ...
          transform: [
            ...
            { rotateZ: `${interpolate(progress.value, [0, 1], [-15, 0])}deg` },
...
const EvaporationTransitioner = ({ text }: { text: string }) => {
  ...
  const renderWords = (textToRender: string, isPrev: boolean) => {
    const totalChars = textToRender.length;
    return (
...
export const LoadingBubble = ({ isUser, agentProgress }: Props) => {
  ...
  return (
    <View style={[styles.bubble, styles.aiBubble]}>
      {agentProgress && <EvaporationTransitioner text={agentProgress} />}
    </View>
  );
};
