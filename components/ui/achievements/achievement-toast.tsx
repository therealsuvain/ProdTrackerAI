  ...
  return (
    <Animated.View style={[styles.wrapper, animatedStyle, { zIndex: 9999 }]}>
      <Surface style={[styles.container, { backgroundColor: theme.colors.elevation.level5 }]} elevation={5}>
        <View style={[styles.iconRing, { borderColor: getTierColor(badge?.tier) }]}>
          <Ionicons name="trophy" size={24} color={getTierColor(badge?.tier)} />
        </View>
        <View style={styles.textContainer}>
          <Text variant="labelMedium" style={{ color: getTierColor(badge?.tier), fontWeight: 'bold' }}>
...
const styles = StyleSheet.create({
  wrapper: {
    ...
    right: 0,
    alignItems: 'center',
    pointerEvents: 'none', // Allows touches to pass through the empty space around the toast
  },
  container: {
    flexDirection: 'row',
    alignItems: 'center',
