...
export default function AchievementBadge({ badge, isUnlocked, unlockedAt, currentProgress }: AchievementBadgeProps) {
  const theme = useTheme();

  const getTierColor = () => {
    if (!isUnlocked) return theme.colors.outlineVariant; // Tone-deaf/muted gray for locked
    switch (badge.tier) {
      case 'diamond': return '#C71585';
    ...
    >
      ...
      <View style={styles.textContainer}>
        <View style={styles.headerRow}>
          <Text variant="titleMedium" style={[styles.title, { color: isUnlocked ? theme.colors.onSurface : theme.colors.onSurfaceVariant }]}>
            {badge.title}
          </Text>
        ...
        {isUnlocked && unlockedAt && (
          <Text variant="labelSmall" style={[styles.date, { color: theme.colors.primary }]}>
